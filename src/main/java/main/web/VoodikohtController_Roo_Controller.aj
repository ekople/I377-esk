...
privileged aspect VoodikohtController_Roo_Controller {
    ...
    public String VoodikohtController.create(@Valid Voodikoht voodikoht, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        ...
        uiModel.asMap().clear();
        voodikoht.persist();
        return "redirect:/voodikohts/" + encodeUrlPathSegment(voodikoht.getId().toString(), httpServletRequest);
    }
    ...
    public String VoodikohtController.show(@PathVariable("id") Long id, Model uiModel) {
        uiModel.addAttribute("voodikoht", Voodikoht.findVoodikoht(id));
    ...
    public String VoodikohtController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        uiModel.addAttribute("voodikoht", Voodikoht.findVoodikoht(id));
        return "voodikohts/update";
    }
    
    @RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
    public String VoodikohtController.delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        Voodikoht.findVoodikoht(id).remove();
        uiModel.asMap().clear();
